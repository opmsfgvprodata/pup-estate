...
namespace MVC_SYSTEM.Controllers
{
    ...
    public class CheckRollGMNController : Controller
    {
                        ...
                        if (!CutOfDateStatus)
                        {
                            if (CustMod_HariTerabai.SelectionCategory == 1)
                            {
                                ...
                                if (datainkrjhdrs.Count() == 0)
                                {
                                    foreach (var pkjid in pkjids)
                                    {
                                        ...
                                        if (CustMod_HariTerabai.JenisChargeHT == "kong")
                                        {
                                            if (!EstateFunction.CheckSAPGLMap2(dbr, pkjid, CustMod_HariTerabai.PilihanAktvtHT, NegaraID, SyarikatID, WilayahID, LadangID, false, "-", out GLCode, out PaySheetID))
                                            {
                            ...
                            else
                            {
                                ...
                                if (datainkrjhdr == null)
                                {
                                    ...
                                    if (!EstateFunction.CheckSAPGLMap2(dbr, pkjdata.fld_Nopkj, CustMod_HariTerabai.PilihanAktvtHT, NegaraID, SyarikatID, WilayahID, LadangID, false, "-", out GLCode, out PaySheetID))
                                    {
                                        msg = "Kod GL tidak dijumpai untuk aktiviti ini.";
                                        statusmsg = "warning";
                                        breakprocess = true;
                            ...
                            if (tbl_Kerjahdrs.Count() != 0 && !breakprocess)
                            {
                                msg = GlobalResEstate.msgAdd;
                                statusmsg = "success";
                                dbr.tbl_Kerjahdr.AddRange(tbl_Kerjahdrs);
            ...
            if (HadirData != null)
            {
                ...
                if (!CutOfDateStatus)
                {
                    ...
                    else
                    {
                        ...
                        {
                            msg = "Kod aktivit ini tidak dibenarkan disimpan";
                            statusmsg = "warning";
                        }
                        else
                        {
        ...
        public JsonResult GetMenuaiRate(DateTime SelectDate, int JnisPkt, string PilihanPkt, string kdhmnuai)
        {
            ...
            kadarharga = EstateFunction.SpecialRateTable(SelectDate, JnisPkt, PilihanPkt, kdhmnuai, dbr, NegaraID, SyarikatID, WilayahID, LadangID, out YieldBracketFullMonth, 0);
            if (kadarharga != 0)
            {
        ...
        public JsonResult GetForm(int SelectionCategory, string SelectionData, string PilihanPkt, string KodAktvt, DateTime SelectDate)
        {
            ...
            var tbl_JenisAktiviti = db.tbl_JenisAktiviti
                ...
                , k => new { k.fld_NegaraID, k.fld_SyarikatID, KodJnsAktvt = k.fld_KategoriAktvt }
                , (j, k) => new {
                    k.fld_NegaraID,
                    k.fld_SyarikatID,
                    j.fld_KodJnsAktvt,
                ...
                .Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_KodJnsAktvt == tbl_UpahAktiviti.fld_KategoriAktvt && x.fld_KodAktvt == KodAktvt && x.fld_Deleted == false).FirstOrDefault();
