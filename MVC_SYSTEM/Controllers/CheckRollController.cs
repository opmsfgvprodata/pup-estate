...
//    public class CheckRollController : Controller
//    {
...
//            ViewBag.CheckRollMenu = new SelectList(db.tblMenuLists.Where(x => x.fld_Flag == "dataEntry" && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fldDeleted == false).OrderBy(o => o.fld_ID), "fld_Val", "fld_Desc");
//            return View();
//        }
...
//                {
//                    dbr.Dispose();
//                    return RedirectToAction("CheckRollBlock", "CheckRoll", new { msg = 1 });
//                }
//                else
//                {
...
//            Connection.GetConnection(out host, out catalog, out user, out pass, WilayahID.Value, SyarikatID.Value, NegaraID.Value);
//            MVC_SYSTEM_Models dbr = MVC_SYSTEM_Models.ConnectToSqlServer(host, catalog, user, pass);
//            List<Models.tbl_Kerjahdr> tbl_Kerjahdrs = new List<Models.tbl_Kerjahdr>();
//            List<Models.tbl_Kerjahdr> returntbl_Kerjahdr = new List<Models.tbl_Kerjahdr>();
...
//                            {
//                                List<string> needtoadds = pkjids.Except(datainkrjhdrs.Select(s => s.fld_Nopkj)).ToList();
//                                if (LeaveSelection)
//                                {
...
//                                    tbl_Kerjahdrs.Add(new Models.tbl_Kerjahdr() { fld_Nopkj = pkjdata.fld_Nopkj, fld_Kum = KumpulanKod.Trim(), fld_Tarikh = CustMod_Attandance.dateseleted, fld_Kdhdct = CustMod_Attandance.WorkCode, fld_Hujan = CustMod_Attandance.Rainning, fld_NegaraID = NegaraID, fld_SyarikatID = SyarikatID, fld_WilayahID = WilayahID, fld_LadangID = LadangID, fld_CreatedBy = getuserid, fld_CreatedDT = date, fld_DataSource = "B" });
//                                    disablesavebtn = true;
...
//                    if (ZeroLeaveBal)
//                    {
//                        msg2 = GlobalResEstate.msgAttendanceLeave + msg2;
//                        statusmsg2 = "Perhatian";
//                    }
//                }
//                else
...
//            PilihanPktHT = new SelectList(dbr.tbl_PktUtama.Where(x => x.fld_Deleted == false && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_Deleted == false).OrderBy(o => o.fld_NamaPktUtama).Select(s => new SelectListItem { Value = s.fld_PktUtama, Text = s.fld_PktUtama + " - " + s.fld_NamaPktUtama }), "Value", "Text").ToList();
...
//            MasterModels.tbl_UpahAktiviti GetActvty = new MasterModels.tbl_UpahAktiviti();
...
//                var pengurus = db.tblUsers.Where(x => x.fldUserName == idpengurus && x.fldUserPassword == passpengurus && x.fldDeleted == false && x.fldRoleID >= 6).SingleOrDefault();
//                if (pengurus != null)
//                {
//                    try
//                    {
//                        if (EstateFunction.CheckSAPGLMap(dbr, CustMod_HariTerabai.JnisPktHT, CustMod_HariTerabai.PilihanPktHT, CustMod_HariTerabai.PilihanAktvtHT, NegaraID, SyarikatID, WilayahID, LadangID, true))
//                        {
...
//                                        foreach (var pkjid in pkjids)
//                                        {
//                                            tbl_Kerjahdrs.Add(new Models.tbl_Kerjahdr() { fld_Nopkj = pkjid, fld_Kum = CustMod_HariTerabai.SelectionData, fld_Tarikh = CustMod_HariTerabai.dateseleted, fld_Kdhdct = CustMod_HariTerabai.WorkCode, fld_Hujan = CustMod_HariTerabai.Rainning, fld_NegaraID = NegaraID, fld_SyarikatID = SyarikatID, fld_WilayahID = WilayahID, fld_LadangID = LadangID, fld_CreatedBy = getuserid, fld_CreatedDT = date, fld_DataSource = "B" });
//                                            if (CustMod_HariTerabai.JenisChargeHT == "kong")
//                                            {
...
//                                        KumpulanKod = dbr.tbl_KumpulanKerja.Where(x => x.fld_KumpulanID == pkjdata.fld_KumpulanID && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_deleted == false).Select(s => s.fld_KodKumpulan).FirstOrDefault();
//                                        tbl_Kerjahdrs.Add(new Models.tbl_Kerjahdr() { fld_Nopkj = pkjdata.fld_Nopkj, fld_Kum = KumpulanKod.Trim(), fld_Tarikh = CustMod_HariTerabai.dateseleted, fld_Kdhdct = CustMod_HariTerabai.WorkCode, fld_Hujan = CustMod_HariTerabai.Rainning, fld_NegaraID = NegaraID, fld_SyarikatID = SyarikatID, fld_WilayahID = WilayahID, fld_LadangID = LadangID, fld_CreatedBy = getuserid, fld_CreatedDT = date, fld_DataSource = "B" });
//                                        if (CustMod_HariTerabai.JenisChargeHT == "kong")
//                                        {
//                                            if (CustMod_HariTerabai.PilihanMasaHT == "separuh")
//                                            {
...
//                                            tbl_Kerjas.Add(new tbl_Kerja() { fld_Amount = Amount, fld_Bonus = 0, fld_BrtGth = 0, fld_CreatedBy = getuserid, fld_CreatedDT = timezone.gettimezone(), fld_DataSource = "B", fld_JamOT = 0, fld_JnisAktvt = GetJenisActvtDetails.fld_KodJnsAktvt, fld_JnsPkt = CustMod_HariTerabai.JnisPktHT, fld_JumlahHasil = Hasil, fld_KadarByr = GetKadarUpah, fld_KdhMenuai = "-", fld_KodAktvt = CustMod_HariTerabai.PilihanAktvtHT, fld_KodGL = "602", fld_KodPkt = CustMod_HariTerabai.PilihanPktHT, fld_Kong = 0, fld_Kum = CustMod_HariTerabai.SelectionData, fld_LadangID = LadangID, fld_NegaraID = NegaraID, fld_SyarikatID = SyarikatID, fld_WilayahID = WilayahID, fld_Nopkj = pkjdata.fld_Nopkj, fld_PerBrshGth = 0, fld_Quality = 0, fld_Tarikh = CustMod_HariTerabai.dateseleted, fld_Unit = GetActvty.fld_Unit, fld_HrgaKwsnSkar = 0, fld_KodKwsnSkar = "-" });
//                                        }
...
//            JnisAktvt = new SelectList(db.tbl_JenisAktiviti.Where(x => x.fld_Deleted == false && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_Deleted == false && x.fld_Lejar == LejarSelect).OrderBy(o => o.fld_KodJnsAktvt).Select(s => new SelectListItem { Value = s.fld_KodJnsAktvt, Text = s.fld_Desc }), "Value", "Text").ToList();
...
//            Connection.GetConnection(out host, out catalog, out user, out pass, WilayahID.Value, SyarikatID.Value, NegaraID.Value);
//            MVC_SYSTEM_Models dbr = MVC_SYSTEM_Models.ConnectToSqlServer(host, catalog, user, pass);
//            if (EstateFunction.CheckSAPGLMap(dbr, JnisPkt, PilihanPkt, PilihanAktvt, NegaraID, SyarikatID, WilayahID, LadangID, false))
//            {
//                if (HadirData != null)
//                {
...
//            List<Models.tbl_Pkjmast> tbl_Pkjmast = new List<Models.tbl_Pkjmast>();
...
//            Models.tbl_Kerjahdr tbl_Kerjahdr = new Models.tbl_Kerjahdr();
//            List<CustMod_WorkerWork> CustMod_WorkerWorks = new List<CustMod_WorkerWork>();
...
//                        CustMod_Kerjahdrs.Add(new CustMod_Kerjahdr() { fld_Nopkj = pkjmast1.fld_Nopkj, fld_Nama = pkjmast1.fld_Nama, fld_Kum = tbl_KumpulanKerja.fld_KodKumpulan, fld_Status = "Tiada rekod", fld_HdrCt = "-", fld_Hujan = "-", fld_GajiTerkumpul = "-" });
...
//                if (datainkrjhdr != null)
//                {
//                    GajiTerkumpul = dbr.tbl_Kerja.Where(x => x.fld_Nopkj == datainkrjhdr.fld_Nopkj && x.fld_Kum == tbl_KumpulanKerja.fld_KodKumpulan && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_Tarikh.Value.Month == SelectDate.Month && x.fld_Tarikh.Value.Year == SelectDate.Year).Sum(s => s.fld_Amount);
//                    GajiTerkumpul = GajiTerkumpul == null ? 0 : GajiTerkumpul;
//                    CustMod_Kerjahdrs.Add(new CustMod_Kerjahdr() { fld_Nopkj = datainpkjmast.fld_Nopkj, fld_Nama = datainpkjmast.fld_Nama, fld_Kum = tbl_KumpulanKerja.fld_KodKumpulan, fld_Status = "Ada rekod", fld_HdrCt = GetConfig.GetWebConfigDesc(datainkrjhdr.fld_Kdhdct, "cuti", (int)NegaraID, (int)SyarikatID), fld_Hujan = datainkrjhdr.fld_Hujan == 0 ? "Tidak" : "Ya", fld_CreatedBy = getidentity.Username2(datainkrjhdr.fld_CreatedBy), fld_CreatedDT = datainkrjhdr.fld_CreatedDT, fld_GajiTerkumpul = GajiTerkumpul.ToString() });
...
//                else
//                {
//                    CustMod_Kerjahdrs.Add(new CustMod_Kerjahdr() { fld_Nopkj = datainpkjmast.fld_Nopkj, fld_Nama = datainpkjmast.fld_Nama, fld_Kum = tbl_KumpulanKerja.fld_KodKumpulan, fld_Tarikh = SelectDate, fld_Status = "Tiada rekod", fld_HdrCt = "-", fld_Hujan = "-", fld_GajiTerkumpul = "-" });
...
//                {
//                    datedisable = false;
//                    CustMod_Kerjahdrs.Add(new CustMod_Kerjahdr() { fld_Nopkj = pkjmast1.fld_Nopkj, fld_Nama = pkjmast1.fld_Nama, fld_Kum = tbl_KumpulanKerja.fld_KodKumpulan, fld_Status = "-", fld_HdrCt = "-", fld_Hujan = "-", fld_GajiTerkumpul = "-" });
//                }
//                else
//                {
...
//                    statusmsg = "warning";
//                    datedisable = true;
//                    CustMod_Kerjahdrs.Add(new CustMod_Kerjahdr() { fld_Nopkj = pkjmast1.fld_Nopkj, fld_Nama = pkjmast1.fld_Nama, fld_Kum = "Tiada Kumpulan", fld_Status = "-", fld_HdrCt = "-", fld_Hujan = "-", fld_GajiTerkumpul = "-" });
//                }
//                if (SelectionCategory != 1)
//                {
...
//                    {
//                        tbl_KumpulanKerja = dbr.tbl_KumpulanKerja.Where(x => x.fld_KumpulanID == pkjmast1.fld_KumpulanID && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_deleted == false).FirstOrDefault();
//                        GajiTerkumpul = dbr.tbl_Kerja.Where(x => x.fld_Nopkj == pkjmast1.fld_Nopkj && x.fld_Kum == tbl_KumpulanKerja.fld_KodKumpulan && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_Tarikh.Value.Month == SelectDate.Month && x.fld_Tarikh.Value.Year == SelectDate.Year).Sum(s => s.fld_Amount);
//                        GajiTerkumpul = GajiTerkumpul == null ? 0 : GajiTerkumpul;
//                        CustMod_Kerjahdrs.Add(new CustMod_Kerjahdr() { fld_Nopkj = pkjmast1.fld_Nopkj, fld_Nama = pkjmast1.fld_Nama, fld_Kum = tbl_KumpulanKerja.fld_KodKumpulan, fld_Status = "Tiada rekod", fld_HdrCt = "-", fld_Hujan = "-", fld_GajiTerkumpul = GajiTerkumpul.ToString() });
//                        namelabel = tbl_KumpulanKerja.fld_Keterangan;
//                        checkxhadir = true;
//                    }
...
//                if (datainkrjhdr != null)
//                {
//                    GajiTerkumpul = dbr.tbl_Kerja.Where(x => x.fld_Nopkj == datainpkjmast.fld_Nopkj && x.fld_Kum == tbl_KumpulanKerja.fld_KodKumpulan && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_Tarikh.Value.Month == SelectDate.Month && x.fld_Tarikh.Value.Year == SelectDate.Year).Sum(s => s.fld_Amount);
//                    GajiTerkumpul = GajiTerkumpul == null ? 0 : GajiTerkumpul;
...
//                    CustMod_Kerjahdrs.Add(new CustMod_Kerjahdr() { fld_Nopkj = datainpkjmast.fld_Nopkj, fld_Nama = datainpkjmast.fld_Nama, fld_Kum = tbl_KumpulanKerja.fld_KodKumpulan, fld_Status = "Ada rekod", fld_HdrCt = GetConfig.GetWebConfigDesc(datainkrjhdr.fld_Kdhdct, "cuti", (int)NegaraID, (int)SyarikatID), fld_Hujan = HariTerabaiStatus, fld_CreatedBy = getidentity.Username2(datainkrjhdr.fld_CreatedBy), fld_CreatedDT = datainkrjhdr.fld_CreatedDT, fld_GajiTerkumpul = GajiTerkumpul.ToString(), fld_UniqueID = datainkrjhdr.fld_UniqueID });
//                    kodhdr = datainkrjhdr.fld_Kdhdct;
//                    kodhjn = datainkrjhdr.fld_Hujan.ToString();
//                    msg = GlobalResEstate.msgDataExist;
//                    statusmsg = "warning";
...
//                    GajiTerkumpul = dbr.tbl_Kerja.Where(x => x.fld_Nopkj == datainpkjmast.fld_Nopkj && x.fld_Kum == tbl_KumpulanKerja.fld_KodKumpulan && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_Tarikh.Value.Month == SelectDate.Month && x.fld_Tarikh.Value.Year == SelectDate.Year).Sum(s => s.fld_Amount);
//                    GajiTerkumpul = GajiTerkumpul == null ? 0 : GajiTerkumpul;
//                    CustMod_Kerjahdrs.Add(new CustMod_Kerjahdr() { fld_Nopkj = datainpkjmast.fld_Nopkj, fld_Nama = datainpkjmast.fld_Nama, fld_Kum = tbl_KumpulanKerja.fld_KodKumpulan, fld_Tarikh = SelectDate, fld_Status = "Tiada rekod", fld_HdrCt = "-", fld_Hujan = "-", fld_GajiTerkumpul = GajiTerkumpul.ToString() });
//                    namelabel = datainpkjmast.fld_Nama;
//                    msg = GlobalResEstate.msgNoRecord;
//                    statusmsg = "success";
//                    disablesavebtn = false;
...
//            string LadangNegeriCode = GetLadang.GetLadangNegeriCode(LadangID);
//            string AttCodeType = GetTriager.GetAttType(LadangNegeriCode.ToString(), NegaraID, SyarikatID, SelectDate);
...
//        public JsonResult GetForm(int SelectionCategory, string SelectionData, int JnisPkt, string PilihanPkt, string JnisAktvt, string KodAktvt, DateTime SelectDate)
//        {
//            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
//            int? getuserid = getidentity.ID(User.Identity.Name);
//            string host, catalog, user, pass = "";
...
//            var tbl_JenisAktiviti = db.tbl_JenisAktiviti.Join(db.tbl_UpahAktiviti, j => j.fld_KodJnsAktvt, k => k.fld_KodJenisAktvt, (j, k) => new { k.fld_NegaraID, k.fld_SyarikatID, j.fld_KodJnsAktvt, j.fld_DisabledFlag, k.fld_Harga, k.fld_KodAktvt, k.fld_KdhByr, k.fld_Unit, j.fld_Deleted, k.fld_MaxProduktiviti }).Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_KodJnsAktvt == JnisAktvt && x.fld_KodAktvt == KodAktvt && x.fld_Deleted == false).FirstOrDefault();
//            //tbl_JenisAktiviti.fld_DisabledFlag - 1 = kong box & kualiti box xde, 2 = kong je xde, 3 = kong je kluar
//            kadarharga = tbl_JenisAktiviti.fld_KdhByr == "B" ? tbl_JenisAktiviti.fld_Harga : EstateFunction.YieldBracket(SelectDate, JnisPkt, PilihanPkt, "A", dbr, NegaraID, SyarikatID, WilayahID, LadangID, out YieldBracketFullMonth);
//            if (kadarharga != 0)
//            {
//                if (SelectionCategory == 1)
//                {
...
//            dbr.Dispose();
//            return Json(new { tablelisting = bodyview, msg, statusmsg, closeform });
//        }
